...
int CN(double t, double dt, double h) // theta = 0
{
	...
	for(int i=1;i<ts;i++)
	{
		...
		{
			u[i][j]= (dt/(h*h))*(u[i-1][j+1]-2*u[i-1][j]+u[i-1][j-1]) + u[i-1][j];
		}
	}
	ofstream file1;
	file1.open("MT1Q4.csv",ios::out);
...
int main()
{
	CN(2,0.001,0.1); //dt = 0.1s & h=0.1 with time=2s as given in question
	return 0;
}
