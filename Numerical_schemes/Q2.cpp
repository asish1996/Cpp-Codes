...
int Upwind(double t, double dt, double h)
{
	...
	for(int i=1;i<ts;i++)
	{
		for(int j=1;j<nx;j++)
		{
			u[i][j]= -(dt/h)*(u[i-1][j]-u[i-1][j-1]) + u[i-1][j];
		}
	}
	ofstream file1;
	file1.open("MT1Q2.csv",ios::out);
...
int main()
{
	Upwind(2,0.1,0.1); //dt = 0.1s & h=0.1 with time=2s as given in question
	return 0;
}
