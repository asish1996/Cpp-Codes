...
int main()
{
				...
				for (int p=0;p<ng;p++){
					for (int alpha=0;alpha<4;alpha++){
						for (int beta=0;beta<4;beta++){
							...
							//reduced stiffness
	double K_red[11][11] = {0};
	double load_vector[11] = {0};
	int nod_req[11] = {3,4,6,7,9,10,11,13,14,15,16};
	double u_n[3] = {0.64, 0.96, 0.96}; //non-zero U's on boundary
		
	for (int i=0;i<11;i++){
		for (int j=0;j<11;j++){
			K_red[i][j] = K_stiff[nod_req[i]-1][nod_req[j]-1];
			load_vector[i] = K_stiff[nod_req[i]-1][1]*u_n[0]+K_stiff[nod_req[i]-1][4]*u_n[1]+K_stiff[nod_req[i]-1][7]*u_n[2];
		}
	}
	...
	ofstream file1;
	ofstream file2;
	file1.open("Q2_Global_Stiffness.csv",ios::out);
	file2.open("Q2_u_required.txt",ios::out);
